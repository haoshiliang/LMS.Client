      ...
      <template v-for="field in this.queryParam.WhereList">
        <div class="searchItem" v-if="field.ControlType=='TextBox' && field.IsDefaultQuery">
          <div class="searchInputDiv">
            <el-input size="small" v-model="field.Value" :class="inputCss" @keyup.enter.native="getList" :placeholder="field.Title"></el-input>
          </div>
          <div style="clear:both;"></div>
        </div>
        <div class="searchItem" v-else-if="field.ControlType=='ComboTreeBox' && field.IsDefaultQuery">
          <div class="searchInputDiv">
            <select-tree v-model="field.Value" :width="inputWidth" size="small" :options="field.BinderList" :targetName="field.TargetName" :change="selectChange"  :placeholder="field.Title"  :props="treeDefaultProps"/>
          </div>
          <div style="clear:both;"></div>
        </div>
        <div class="searchItem" v-else-if="field.ControlType=='ComboRadioBox' && field.IsDefaultQuery">
          <div class="searchInputDiv">
            <el-select size="small" clearable v-model="field.Value"  :placeholder="field.Title" :class="inputCss" @change="selectChange(field.Value,field.TargetName)">
        ...
        <div class="searchItem" v-else-if="field.ControlType=='ComboMultipleBox' && field.IsDefaultQuery">
          <div class="searchInputDiv">
            <el-select size="small" clearable multiple v-model="field.Value"  :placeholder="field.Title" :class="inputCss" @change="selectChange(field.Value,field.TargetName)">
              <el-option v-for="item in field.BinderList":key="item.Id" :label="item.Name" :value="item.Id"></el-option>
            </el-select>          </div>
          <div style="clear:both;"></div>
        </div>
        <div class="searchItem" v-else-if="field.ControlType=='TimeText' && field.IsDefaultQuery">
          <div class="searchInputDiv">
