...
<script>
    export default {
      ...
      methods: {
        submitForm: function () {
          let param = Object.assign([], this.functionList);
          this.$ajax({
            method: "post",
            url: "/api/Function",
            data: param
        ...
        setAddForm: function (mId,editFormData) {
          this.mId = mId;
          this.functionList = editFormData;
          this.addFormVisible = true;
        },
        addRow:function () {
          this.functionList.push({Id:'',Name:'',ParentId:this.mId,IsEnabled:true});
        },
        delRow:function () {
          if (this.selectedRow != null) {
            this.$confirm("确认删除该记录吗?", "提示", {type: "warning"})
              .then(() => {
                let fList=this.functionList;
